...
class GraphMaker:
    ...
    def make_graph(self):
        thermo_df = pd.DataFrame(self.thermo_data, columns = self.thermo_keywords)
        #color = []
        #markerstyle = []
        #linestyle = []
        #legend = False
        ...
        #ciclo while per scegliere colonne da plottare + in che modo
    ...
    def interact_graph(self):
        ...
        print('The following quantities have been found: ' + str(self.thermo_keywords)[1:-1])
        while True:
            keyword_check = True
            ...
            if prova[0] == 'display':
                ...
                for keyword in keywords_list:
                    if keyword not in self.thermo_keywords:
                        ...
                        keyword_check = False
                if keyword_check:        
                    self.make_graph(keywords_list,self.thermo_keywords,self.thermo_data)
            elif graph == 'exit':
                exit
            else:
